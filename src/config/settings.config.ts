...
export const motionBlurQualityList = Object.keys(motionBlurQualityMap) as MotionBlurQualityKey[];
...
function mergeParams(search: URLSearchParams, hash: URLSearchParams): URLSearchParams {
...
function parseAmount(params: URLSearchParams): {
  ...
  const amount: AmountKey = amountList.includes(amountParam) ? amountParam : '65k';
...
function parseMotionBlurQuality(params: URLSearchParams): MotionBlurQualityKey {
  const mbqParam = (params.get('motionBlurQuality') || 'medium') as MotionBlurQualityKey;
