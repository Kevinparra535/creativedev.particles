...
const coord3To2 = glsl`
vec2 coord3To2(vec3 texCoord, float size, float slicesPerRow) {
  float slice = clamp(texCoord.z * size, 0.0, size - 1.0);
  float sliceCol = mod(slice, slicesPerRow);
  float sliceRow = floor(slice / slicesPerRow);
  // tamaño de cada tile (sub-textura) dentro del atlas
  float tileSizeX = 1.0 / slicesPerRow;
  // filas totales realmente usadas
  float numRows = floor((size + slicesPerRow - 1.0) / slicesPerRow);
  float tileSizeY = 1.0 / numRows;

  // UV finales dentro del tile correspondiente
  return vec2(
    (sliceCol + texCoord.x) * tileSizeX,
    (sliceRow + texCoord.y) * tileSizeY
  );
}
`;

const sampleAs3DTexture = glsl`
vec4 sampleAs3DTexture(sampler2D tex, vec3 texCoord, float size, float slicesPerRow) {
  vec2 uv = coord3To2(texCoord, size, slicesPerRow);
  return texture(tex, uv);
}
`;

const simplexNoiseDerivatives4 = glsl`
...
// (Exporta también por separado si quieres importar solo uno)
export {
  coord2To3,
  coord3To2,
  sampleAs3DTexture,
