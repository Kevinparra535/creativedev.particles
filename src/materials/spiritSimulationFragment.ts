...
// --- Legacy-inspired curl noise using simplex derivatives (4D) ---
...
vec3 curl(vec3 p, float noiseTime, float persistence) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    xDer += simplexNoiseDerivatives(vec4(p * twoPowI, noiseTime)) * scale;
    yDer += simplexNoiseDerivatives(vec4((p + vec3(123.4, 129845.6, -1239.1)) * twoPowI, noiseTime)) * scale;
    zDer += simplexNoiseDerivatives(vec4((p + vec3(-9519.0, 9051.0, -123.0)) * twoPowI, noiseTime)) * scale;
  }
  return vec3(
    zDer[1] - yDer[2],
    xDer[2] - zDer[0],
